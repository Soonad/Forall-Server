defmodule Forall.FilesTest do
  ...
  describe "get_file/2" do
    test "should return nil if no file is found" do
      assert {:error, :not_found} = Files.get_file(file_name(), file_version())
    end
    ...
    test "should return the file" do
      ...
      assert {:ok, returned_file = %File{}} = Files.get_file(file.name, file.version)

      assert returned_file.name == file.name
      assert returned_file.version == file.version
